...
use ckb_types::{
    ...
    packed::{Byte32, CellDep, CellOutput, OutPoint, Script, OutPointVec},
    prelude::*,
};
...
impl Context {
    ...
    fn build_resolved_tx(&self, tx: &TransactionView) -> ResolvedTransaction {
        ...
        for deps_out_point in tx.cell_deps().into_iter() {
            ...
            // Extract cells from dep_group cell_dep.
            if DepType::try_from(deps_out_point.dep_type()).unwrap() == DepType::DepGroup {
                let out_points = OutPointVec::from_slice(dep_data.as_ref()).expect("Expect valid dep_group data.");

                for deps_out_point in out_points.into_iter() {
                    let (dep_output, dep_data) = self.cells.get(&deps_out_point).unwrap();
                    let tx_info_opt = self.transaction_infos.get(&deps_out_point);
                    let mut b = CellMetaBuilder::from_cell_output(
                        dep_output.to_owned(),
                        dep_data.to_vec().into(),
                    )
